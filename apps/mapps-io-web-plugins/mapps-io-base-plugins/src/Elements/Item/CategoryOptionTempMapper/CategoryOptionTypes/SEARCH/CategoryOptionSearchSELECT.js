...
function CategoryOptionSearchSELECT(props) {
  ...
  const [currentValue, setCurrentValue] = React.useState(optionFilter[0] && optionFilter[0].id);
  ...
  React.useEffect(() => {
    if (isUuid(currentValue)) {
      debounceFunc(currentValue, (cr) => {
        ...
        obj.catOptions[props.catOptionId + "_SELECT"] = [
          {
            id: cr
          }
        ];
  ...
  function resetSingle() {
    const obj = {
      catOptions: {
        ...props.filterSearchReducer.search.catOptions,
        [props.catOptionId + "_SELECT"]: undefined
      }
    };
  ...
  return (
    <FormControl component="fieldset" style={{ width: "100%" }}>
      ...
      <RadioGroup value={currentValue} onChange={onClick}>
