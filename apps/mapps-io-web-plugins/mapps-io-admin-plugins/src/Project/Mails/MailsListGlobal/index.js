...
class MailsListGlobal extends React.Component {
  constructor(props) {
    super(props);
    this.state = { filter: "" };
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <List>
          <Link to={`/mapps/mails/global/new`}>
          ...
          {this.props.mailList.mail_types_global.map((item) => {
            return (
              <Link key={uuid()} to={`/mapps/mails/global/edit/${item.id}`}>
                <ListItem button>
...
export default connect(mapStateToProps, mapDispatchToProps)(MailsListGlobal);
